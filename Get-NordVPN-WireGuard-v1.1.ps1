# ==============================================================================
# Script: Get-NordVPN-WireGuard-v1.1.ps1
# Version: 1.1
...
function Invoke-Cleanup {
    Remove-Variable -Scope Script -Name Token, credentials, encodedCredentials, headers, privKeyResponse, privateKey, confContent -ErrorAction SilentlyContinue
    [System.GC]::Collect()
}
...
Write-Host '    ║    // /   \ \\   C O N F I G   G E N E R A T O R   v1.1   ║' -ForegroundColor DarkCyan
Write-Host '    ║   <<_/     \_>>                                           ║' -ForegroundColor DarkCyan
Write-Host '    ║                                                           ║' -ForegroundColor DarkCyan
...
"@
        ...
        try {
            Set-Content -Path $filePath -Value $confContent -Encoding Ascii -ErrorAction Stop

            # [S1] Restrict file permissions to current user only (Windows)
            try {
